...
const paragraphToInline = (str, allowLineBreaks = false) => {
	const wrappers = /^(<p>)|(<\/p>)$/g
	const breaks = /(<\/p>){1}[\s]*(<p>){1}/g
	const absoluteLocals = /(href=")(http){1}(s)?(:\/\/){1}(lameass\.recipes)/g

	return str
		.replace(wrappers, '')
		.replace(breaks, allowLineBreaks ? '<br /><br />' : ' ')
		.replace(absoluteLocals, '$1')
}
...
export const getAllRecipes = async () => {
	const data = await fetchAPI(
		`
      query getAllRecipes {
        allRecipes(orderBy: createdAt_DESC, first: 100) {
          ${recipeFieldsFragment}
        }
        ${siteSEOFragment}
      }
...
export const getRecipeBySlug = async (slug) => {
	...
	const ingredients = data?.recipe?.recipe
		...
		.map((i) => ({
			id: i.id,
			value: paragraphToInline(i.name),
		}))
	...
	const steps = data?.recipe?.recipe
		...
		.map((s) => ({
			id: s.id,
			value: paragraphToInline(s.step, true),
		}))
...
export const getAllCategories = async () => {
	const data = await fetchAPI(
      ...
      query getAllCategories {
        allCategories(orderBy: title_ASC, first: 50) {
          id
          title
          emoji
          slug
...
export const getAllRecipesByCategory = async (slug) => {
	...
	const data = await fetchAPI(
      ...
      query getAllRecipesByCategory($category: ItemId!) {
        allRecipes(filter: { category: { eq: $category }}, first: 100) {
          ${recipeFieldsFragment}
        }
        ${siteSEOFragment}
      }
...
export const getAllRecipesByTag = async (slug) => {
	...
	const data = await fetchAPI(
      ...
      query getAllRecipesByTag($tag: ItemId!) {
        allRecipes(filter: { tags: { allIn: [$tag] }}, first: 100) {
          ${recipeFieldsFragment}
        }
        ${siteSEOFragment}
      }
