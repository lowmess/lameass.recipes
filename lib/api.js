...
export const getHomepage = async () => {
	...
	return {
		...
		featuredMealHeading: smartypants(data.featuredMealHeading),
...
export const getMealBySlug = async (slug) => {
	// Note that we're getting recipes again here. It will override the one set
	// in `mealFields`, and allow us to get the equipment each meal requires.

	const data = await client.fetch(
		`
			*[_type == 'meal' && slug.current == $slug] {
				${mealFields},
				recipes[]->{${recipeFields},equipment},
				'yieldAmount': yield_amount,
				sections[] { _key, title, steps }
			}[0]
		`,
